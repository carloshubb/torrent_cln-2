...
class FetchTrendingPage extends Command
{
    ...
    private function saveTorrent(array $torrents): int
    {
        ...
        foreach ($torrents as $torrentData) {
            try {
                ...
                $torrent = Torrent::where('slug', $slug)->first();
                if ($torrent == null) {
                    $torrent = new Torrent();
                ...
                $this->info(date("Y-m-d H:i:s")." $savedCount =>Added or Saved Trending Torrent");
    ...
    private function trendingCategoryInfoPage($userurl)
    {
